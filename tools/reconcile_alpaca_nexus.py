...
def main():
    ...
    cur.execute(
        "SELECT symbol, status, details, timestamp FROM audit_log ORDER BY timestamp DESC"
    )
    audits = cur.fetchall()
    latest_audit = {}
    ...
    cur.execute(
        "SELECT symbol, side, qty, price, status, timestamp FROM trade_history ORDER BY timestamp DESC"
    )
    trades = cur.fetchall()
    latest_trade = {}
    ...
    print(
        "  3) Adjust governance limits (e.g., increase NEXUS_MAX_POSITION_SIZE) and allow new trades"
    )
