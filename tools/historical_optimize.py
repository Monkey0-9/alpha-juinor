...
def main():
    ...
    logger.info(
        f"Downloading historical index data for {symbol} from {start_date} to {end_date}..."
    )
    ...
    logger.info(
        f"Successfully loaded {len(df):,} daily bars of S&P 500 historical data (~{len(df)//252} years)."
    )
    ...
    logger.info(
        "Beginning parameter optimization grid search over historical regimes..."
    )

    total_combinations = (
        len(atr_multipliers)
        * len(profit_locks)
        * len(hurst_thresholds)
        * len(kalman_noises)
    )
    idx = 0
        ...
        for lock in profit_locks:
            for hurst in hurst_thresholds:
                for k_noise in kalman_noises:
                    ...
                    sharpe = (np.mean(simulated_returns) * 252) / (
                        annual_vol if annual_vol > 0 else 1e-9
                    )
                    ...
                    if idx % 20 == 0 or idx == total_combinations:
                        logger.info(
                            f"Progress: {idx}/{total_combinations} combinations evaluated."
                        )
    ...
    logger.info(
        "Successfully updated '.env' file with 100-year optimized hedge-fund parameters!"
    )
