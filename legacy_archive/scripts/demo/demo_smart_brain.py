...
def demonstrate_smart_brain_reasoning():
    ...
    print(f"  Momentum (10-day): {market_state.momentum:.4f}")
    print(f"  Trend Strength: {market_state.trend_strength:.2f}")
    print(f"  Regime: {market_state.regime}")

    decision1 = smart_brain.think(
        market_state=market_state, historical_close_prices=close_prices
    )
    ...
    if decision1["alternate_views"]:
        print("\nAlternate Views:")
        for view in decision1["alternate_views"]:
            print(f"  • {view}")

    # Scenario 2: Market Crash / Crisis
    print("\n" + "-" * 80)
    print("SCENARIO 2: CRISIS REGIME (Market Crash)")
    print("-" * 80)
    ...
    market_state = MarketState(
        ...
        volume=volumes[-1] * 2,  # High volume during crash
        volatility=np.std(np.diff(np.log(close_prices)))
        * np.sqrt(252)
        * 2,  # Higher vol
        momentum=float((close_prices[-1] - close_prices[-10]) / close_prices[-10]),
        trend_strength=0.9,  # Very strong downtrend
        regime="crisis",
        anomaly_score=0.3,  # High anomaly
    )

    print(f"\nMarket State:")
    print(f"  Price: ${market_state.close:.2f}")
    print(f"  Volatility: {market_state.volatility:.4f} (ELEVATED)")
    print(f"  Momentum (10-day): {market_state.momentum:.4f} (NEGATIVE)")
    print(f"  Trend Strength: {market_state.trend_strength:.2f}")
    print(f"  Regime: {market_state.regime} - ALERT!")
    print(f"  Anomaly Score: {market_state.anomaly_score:.2f}")

    decision2 = smart_brain.think(
        market_state=market_state, historical_close_prices=close_prices
    )
    ...
    for i in range(10):
        ...
        market_state = MarketState(
            ...
            regime=np.random.choice(["low_risk", "normal", "high_risk"]),
            anomaly_score=np.random.uniform(0.05, 0.3),
        )

        decision = smart_brain.think(
            market_state=market_state, historical_close_prices=close_prices
        )
        ...
        profit_loss = np.random.uniform(-2, 2)  # Random P&L
