...
from mini_quant_fund.utils.logging_config import setup_logging
...
class AutonomousTradingOrchestrator:
    ...
    def alpha_generator(self):
        ...
        if self._alpha_generator is None:
            from mini_quant_fund.alpha_families.full_market_alpha import get_market_alpha_generator
    ...
    def apply_additional_models(self, symbols: List[str], market_data: Dict):
        ...
        if self.config.get("models_enabled", {}).get("hmm", True):
            try:
                from mini_quant_fund.ml.hmm_predictor import get_hmm_predictor
