...
class AlpacaExecutionHandler:
    ...
    def __init__(
        ...
        # Institutional Rule: Default to Paper but allow Live override via ENV
        env_url = os.environ.get("ALPACA_BASE_URL")
        if env_url:
            self.base_url = env_url
        else:
            # Automatic selection based on TRADING_MODE
        ...
        try:
            ...
            logger.info(f"Connected to Alpaca: {status} | Equity: ${equity}")
    ...
    def _request_with_retry(self, method: str, url: str, **kwargs) -> requests.Response:
        ...
        if response.status_code >= 400:
            err_msg = (
                f"Alpaca API Error {response.status_code}: " f"{response.text}"
            )
    ...
    def get_account(self) -> Dict:
        """Get account details."""
        r = self._request_with_retry("GET", f"{self.base_url}/account")
        return r.json()

    def get_positions(self) -> Dict[str, float]:
        """Get current positions as {ticker: quantity}."""
        r = self._request_with_retry("GET", f"{self.base_url}/positions")
        data = r.json()
        positions = {}
        for pos in data:
    ...
    def submit_orders(self, orders: List[Order]) -> List[Dict]:
        ...
        for order in orders:
            ...
            ticker = (
                order.ticker.replace("-", "") if "-" in order.ticker else order.ticker
            )
            ...
            # IDEMPOTENCY: Use UUID for client_order_id
            # to prevent double-fills on retry
            ...
            try:
                ...
                r = self._request_with_retry(
                    "POST", f"{self.base_url}/v2/orders", json=payload
                )
                order_data = r.json()
                ...
                logger.info(
                    f"Submitted {side} {qty:.4f} {order.ticker} | "
                    f"{reason_log} | ID: {order_data.get('id')} | "
                    f"CID: {client_order_id}"
                )
                results.append({"status": "submitted", "order": order_data})
            except Exception as err:
                logger.error(
                    f"Failed to submit {order.ticker} " f"({order.reason}): {err}"
                )
    ...
    def submit_order(
        ...
        """
        Robust submit: checks buying power,
        disallows fractional short sells.
        ...
        try:
            ...
            ticker = (
                symbol.replace("-", "")
                if "-" in symbol and "USD" not in symbol
                else symbol
            )
                    ...
                    if notional_required + safety_buffer > buying_power:
                        return {
                            ...
                            "detail": {
                                ...
                                "required": notional_required + safety_buffer,
                            },
                            "mapped_symbol": ticker,
                        }
                else:
            ...
            if is_sell:
                ...
                try:
                    ...
                    if resp.status_code != 200:
                        ...
                        shorting_allowed = (
                            acct.get("shorting_enabled", False)
                            or float(acct.get("margin_balance", 0)) != 0
                        )
            ...
            for attempt in range(retries):
                try:
                    ...
                    logger.info(
                        f"Submitted {side} {qty_to_send} {ticker} | "
                    ...
                    last_exc = exc
                    logger.warning(
                        f"Alpaca submission attempt {attempt+1} failed: {exc}"
                    )
            ...
            logger.error(f"Specific error occurred: {specific_error}")
            return {"success": False, "order": None, "error": "Invalid input"}
