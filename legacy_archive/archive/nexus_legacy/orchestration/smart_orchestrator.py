...
class SmartOrchestrator:
    ...
    def _ensure_modules(self):
        ...
        try:
            from mini_quant_fund.ml.bayesian_nn import get_bayesian_nn
            self._bnn = get_bayesian_nn()
        except Exception as e:
            logger.warning(f"BNN not available: {e}")

        try:
            from mini_quant_fund.ml.online_meta_learner import get_meta_learner
        ...
        try:
            from mini_quant_fund.execution.tca_engine import get_tca_engine
        ...
        try:
            from mini_quant_fund.research.advanced_analytics import get_analytics_engine
            self._analytics = get_analytics_engine()
        except Exception as e:
            logger.warning(f"Analytics not available: {e}")

        try:
            from mini_quant_fund.data.alternative_data import get_alt_data_engine
            self._alt_data = get_alt_data_engine()
        except Exception as e:
            logger.warning(f"Alt data not available: {e}")

        try:
            from mini_quant_fund.agents.llm_trade_analyzer import LLMTradeAnalyzer
            self._llm_analyzer = LLMTradeAnalyzer()
        except Exception as e:
            logger.warning(f"LLM analyzer not available: {e}")

        try:
            from mini_quant_fund.production.safety_guards import get_safety_guard
