...
from risk.portfolio_cvar import PortfolioRiskModel
...
class TestTailEvents(unittest.TestCase):
    ...
    def test_synthetic_crash_rejection(self):
        """
        Test that PM Brain rejects trades that increase risk during a synthetic crash.
        ...
        alphas = {
            'TCKR': AlphaDistribution(mu=0.05, sigma=0.02, p_loss=0.4, cvar_95=-0.08, confidence=0.9)
        }
        ...
        # In PM Brain, if we had full integration, it would see the high sigma used in alpha
        # and checking marginal CVaR.
        ...
        stress_alpha = AlphaDistribution(mu=0.01, sigma=0.05, p_loss=0.4, cvar_95=-0.15, confidence=0.9)
        ...
        self.assertEqual(score, 0.0, "PM Brain must block allocation when CVaR (-15%) exceeds limit")
