...
async def test_engine_no_trade_state():
    ...
    engine.alpha_engine.fetch_market_data = AsyncMock(return_value=pd.DataFrame({"close": [150, 151, 152]}))
    ...
    with patch.object(engine.ensemble_brain, 'get_signal', return_value=0.0):
        ...
        with patch.object(engine, 'get_positions', new_callable=AsyncMock) as mock_pos:
            ...
            with patch.object(engine, 'get_account_state', new_callable=AsyncMock) as mock_acc:
                mock_acc.return_value = {"equity": 10000, "buying_power": 10000}
                
                with patch.object(engine, '_submit_trade', new_callable=AsyncMock) as mock_submit:
                    await engine.run_cycle()
                    
                    # Submit trade should NOT be called for AAPL because conviction was 0
...
async def test_engine_crisis_correlation():
    ...
    engine.alpha_engine.fetch_market_data = AsyncMock(return_value=pd.DataFrame({"close": volatile_close}))
    ...
    with patch.object(engine.ensemble_brain, 'get_signal', return_value=0.9):
        with patch.object(engine, 'get_positions', new_callable=AsyncMock) as mock_pos:
            ...
            with patch.object(engine, 'get_account_state', new_callable=AsyncMock) as mock_acc:
                ...
                with patch.object(engine.regime_detector, 'detect', return_value="TURBULENT"):
                    with patch.object(engine, '_submit_trade', new_callable=AsyncMock) as mock_submit:
                        # Also patch the _get_client to avoid http requests for clock and fills
                        ...
                        with patch.object(engine, '_get_client', return_value=mock_client):
                            with patch.object(engine, 'refresh_universe', new_callable=AsyncMock):
                                await engine.run_cycle()
