...
# Setup path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "nexus", "cpp_extensions")))
...
def pure_python_survival(initial_capital, mu, sigma, days, n_simulations, ruin_threshold):
    if sigma == 0:
        return 1.0
...
def run_benchmark():
    ...
    # 1. C++ Benchmark
    start_cpp = time.time()
    surv_cpp = nexus_cpp.run_survival_analysis(initial_capital, mu, sigma, days, n_simulations, ruin_threshold)
    end_cpp = time.time()
    
    # 2. Python Benchmark
    start_py = time.time()
    surv_py = pure_python_survival(initial_capital, mu, sigma, days, n_simulations, ruin_threshold)
    end_py = time.time()
