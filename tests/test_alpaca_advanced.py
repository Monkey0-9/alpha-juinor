...
async def test_alpaca_rate_limit_retry():
    ...
    service = AlpacaClient(credentials=AlpacaCredentials(api_key="TEST", api_secret="TEST"))
    ...
    mock_session.get.return_value = MockAsyncContextSequence([mock_response_429, mock_response_200])
    
    with patch.object(service, "_get_session", new_callable=AsyncMock) as mock_get_session:
...
async def test_alpaca_connection_error_fallback():
    ...
    mock_session = MagicMock()
    exc = aiohttp.ClientConnectorError(connection_key=MagicMock(), os_error=OSError("Network down"))
