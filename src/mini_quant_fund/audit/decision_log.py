...
# Windows path sanitization constants
...
def _sanitize_path(path: str) -> str:
    """
    Sanitize a file path for Windows compatibility.
    Replaces illegal characters and enforces length limits.
    """
    ...
    sanitized_name = re.sub(WINDOWS_ILLEGAL_CHARS, "_", name)
    sanitized_name = sanitized_name.strip(" .")
    if not sanitized_name:
        sanitized_name = "_unnamed_"

    sanitized_filename = sanitized_name + ext
    sanitized_path = (
        os.path.join(directory, sanitized_filename) if directory else sanitized_filename
    )

    # Enforce length limit
    if len(sanitized_path) > MAX_WINDOWS_PATH_LENGTH:
        available_length = MAX_WINDOWS_PATH_LENGTH - len(directory) - len(ext) - 1
        if available_length < 10:
            sanitized_path = sanitized_path[: MAX_WINDOWS_PATH_LENGTH - len(ext)] + ext
        else:
            sanitized_path = os.path.join(
                directory, sanitized_name[:available_length] + ext
            )
...
AUDIT_JSONL_PATH = os.path.join(AUDIT_DIR, "mini_quant_fund.audit.log")
AUDIT_DB_PATH = os.path.join(AUDIT_DIR, "mini_quant_fund.audit.db")
...
def _init_db():
    ...
    try:
        ...
        if os.path.exists(schema_path):
            ...
            logger.warning(
                f"Schema file not found at {schema_path}, using inline schema"
            )
            cursor.execute(
                """
...
def _internal_write_audit(record):
    ...
    with lock:
        ...
        try:
            ...
            # Use sanitized path for Windows compatibility
            safe_path = _sanitize_path(AUDIT_JSONL_PATH)
            with open(safe_path, "a", encoding="utf-8") as f:
                f.write(line + "\n")
        except OSError as e:
            # Handle Windows-specific path errors gracefully
            logger.error(f"AUDIT JSONL WRITE FAILED (OSError): {e}")
            # Try fallback with sanitized timestamp-based filename
            try:
                fallback_name = f"audit_fallback_{pd.Timestamp.now('UTC').strftime('%Y%m%d_%H%M%S')}.log"
                fallback_path = os.path.join(AUDIT_DIR, fallback_name)
                with open(fallback_path, "a", encoding="utf-8") as f:
                    f.write(line + "\n")
                logger.warning(f"AUDIT: Wrote to fallback path {fallback_path}")
            except Exception as fallback_e:
                logger.critical(
                    f"CRITICAL AUDIT JSONL FAILURE (fallback also failed): {fallback_e}"
                )
        except Exception as e:
            logger.error(f"AUDIT JSONL WRITE ERROR: {e}")
        ...
        try:
            ...
            if hasattr(record, "cycle_id"):
                ...
                final_decision = _map_execdecision_to_final(
                    record.decision
                ...
                # Quantum Fields
                quantum_state = (
                    json.dumps(record.quantum_state) if record.quantum_state else "{}"
                )
                regime = (
                    record.regime
                    if isinstance(record.regime, str)
                    else json.dumps(record.regime)
                )
                ...
                raw_decision = record_dict.get(
                    "decision", record_dict.get("final_decision", "ERROR")
                )
                final_decision = _map_execdecision_to_final(raw_decision)

                reason_codes = json.dumps(record_dict.get("reason_codes", []))
                order_data = (
                    json.dumps(record_dict.get("order"))
                    if record_dict.get("order")
                    else None
                )
                ...
                regime = (
                    _regime_raw
                    if isinstance(_regime_raw, str)
                    else json.dumps(_regime_raw)
                )
