...
from mini_quant_fund.risk.capital_stress import CapitalStressTester
from mini_quant_fund.risk.market_impact_models import ImpactParameters, TransactionCostModel
...
class ExecutionGatekeeper:
    ...
    def is_market_open(self) -> bool:
        ...
        try:
            ...
            # FIX: Ensure now.time() returns a time object, not datetime
            current_time = now.time()
            return market_open <= current_time <= market_close
        except Exception as e:
            logger.error(f"Error checking market hours: {e}")
            # CRITICAL FIX: Conservative default = FALSE (don't trade if check fails)
            logger.critical(
                "Market hours check FAILED - defaulting to MARKET CLOSED for safety"
            )
            return False
    ...
    def validate_execution(
        ...
        if not gov or gov["state"] != "ACTIVE":
            ...
            logger.warning(
                f"GOVERNANCE_GATE_TRIGGER: {symbol} is {state} - REJECTING EXECUTION"
            )
        ...
        if not self.is_market_open():
            logger.warning(
                f"MARKET_GATE_TRIGGER: Market is CLOSED - REJECTING EXECUTION for {symbol}"
            )
        ...
        if abs(qty) > max_qty:
            logger.warning(
                f"ADV_GATE_TRIGGER: {symbol} qty={qty} > max_adv_qty={max_qty} (10% of {adv_30d})"
            )
            # Scaledown policy
        ...
        if (
            ...
            logger.warning(
                f"SENTIMENT_GATE_TRIGGER: {symbol} sentiment={sentiment_score} (Extreme Negative) - Blocking BUYS"
            )
        ...
        if impact_bps > self.max_impact_bps:
            logger.warning(
                f"IMPACT_GATE_TRIGGER: {symbol} estimated_impact={impact_bps:.2f}bps > limit={self.max_impact_bps}bps"
            )
