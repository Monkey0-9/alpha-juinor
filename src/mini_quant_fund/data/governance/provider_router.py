...
class ProviderCapabilityMatrix:
    ...
    CAPABILITIES = {
        ...
        PROVIDER_ALPHAVANTAGE: {
            "supported_assets": {ASSET_STOCK, ASSET_CRYPTO, ASSET_FOREX},
            "max_history_days": 7300, # 20 years
            "requires_key": True,
            "cost": "low",
            "priority": 4 # Fallback after Yahoo
        },
        PROVIDER_STOOQ: {
            "supported_assets": {ASSET_STOCK, ASSET_CRYPTO, ASSET_FOREX, ASSET_FUTURES},
            "max_history_days": 36500, # Very long
            "requires_key": False,
            "cost": "free",
            "priority": 5 # Final free fallback
        }
    }
    ...
    def get_eligible_providers(cls, symbol: str, history_days: int) -> List[str]:
        ...
        for provider, caps in cls.CAPABILITIES.items():
            ...
            # 2. Check Key Presence if required
            if caps.get("requires_key", False):
                if provider == PROVIDER_ALPACA:
                    if not os.environ.get("ALPACA_API_KEY") or not os.environ.get("ALPACA_SECRET_KEY"):
                        continue
                elif provider == PROVIDER_ALPHAVANTAGE:
                    if not os.environ.get("ALPHAVANTAGE_API_KEY"):
                        continue
                elif provider == PROVIDER_IBKR:
                    # Logic for IBKR keys
                    continue

            # 3. Check Asset Class Support
