...
from mini_quant_fund.config.secrets_manager import secrets
...
class EntitlementRouter:
    ...
    def _init(self):
        ...
        self.provider_priority = ["bloomberg", "refinitiv", "yahoo", "polygon", "alpaca"] # Yahoo prioritized for tests
        self.rate_limits = {
    ...
    def select_provider(self, symbol: str, required_history_days: int) -> Dict[str, str]:
        ...
        for provider in self.provider_priority:
            ...
            if not self.is_entitled(provider):
                logger.info(f"Provider {provider} not entitled, skipping")
                continue
