...
from mini_quant_fund.data.providers.bloomberg import BloombergDataProvider
from mini_quant_fund.data.providers.coingecko import CoinGeckoDataProvider
from mini_quant_fund.data.providers.finnhub import FinnhubDataProvider
...
from mini_quant_fund.data.providers.reuters import ReutersDataProvider

# Alternative Data Providers
from mini_quant_fund.data.providers.sentiment_provider import SentimentDataProvider
from mini_quant_fund.data.providers.twelvedata import TwelveDataDataProvider
from mini_quant_fund.data.providers.yahoo import YahooDataProvider
from mini_quant_fund.data.providers.alpha_vantage import AlphaVantageProvider
from mini_quant_fund.data.providers.stooq import StooqProvider
from mini_quant_fund.database.manager import DatabaseManager
from mini_quant_fund.utils.timeutils import ensure_business_days
...
from mini_quant_fund.data.governance.provider_router import (
    select_provider as governance_select_provider,
    mark_provider_unavailable,
    PROVIDER_ALPACA, PROVIDER_YAHOO
)
from mini_quant_fund.data.intelligence.confidence_manager import ConfidenceManager
...
class DataRouter:
    ...
    def __init__(self, max_workers: int = 10, enable_cache: bool = True):
        ...
        self.providers = {
            ...
            "alpha_vantage": AlphaVantageProvider(enable_cache=enable_cache),
            "stooq": StooqProvider(enable_cache=enable_cache),
        }
    ...
    def get_price_history(
        ...
        for attempt in range(max_retries):
            try:
                ...
                p_name = next((k for k, v in self.providers.items() if v == provider), "unknown")
                if p_name != "unknown":
                    self._unavailable_cache[p_name] = self._unavailable_cache.get(p_name, 0) + 1
    ...
    def get_panel_parallel(
        self, tickers: List[str], start_date: str, end_date: str = None, allow_long_history: bool = False
    ) -> pd.DataFrame:
        """Fetch multiple tickers in parallel using multithreading."""
        tasks = [(tk, start_date, end_date, allow_long_history) for tk in tickers]

        def _fetch_wrap(args):
            tk, sd, ed, alh = args
            return tk, self.get_price_history(tk, sd, ed, allow_long_history=alh)
    ...
    def get_daily_prices(self, symbol: str, limit: int = 252) -> pd.DataFrame:
        ...
        """
        # Try to load from mini_quant_fund.database first
        try:
            from mini_quant_fund.database.manager import DatabaseManager
            db = DatabaseManager()
...
DATA_PURPOSE_PROVIDER_MAP = {
    "5y_history": {
        "providers": ["yahoo", "stooq", "alpha_vantage", "polygon"],
