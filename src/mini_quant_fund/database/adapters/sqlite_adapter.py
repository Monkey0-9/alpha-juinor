...
class SQLiteAdapter(DatabaseAdapter):
    ...
    def _serialize_json(self, data: Any) -> str:
        """Robust JSON serialization for numpy/pandas types."""
        ...
        return json.dumps(data, cls=InstitutionalEncoder)
    ...
    def log_data_quality(self, record: DataQualityRecord) -> bool:
        ...
        try:
            ...
            val_json = self._serialize_json(record.validation_flags)
    ...
    def upsert_symbol_governance(self, record: SymbolGovernanceRecord) -> bool:
        ...
        try:
            ...
            ''', (record.symbol, record.state, record.data_quality, record.reason,
                  ...
                  self._serialize_json(record.metadata) if record.metadata else None))
            conn.commit()
            return True
        except Exception as e:
    ...
    def insert_model_outputs(self, outputs: List) -> int:
        ...
        conn = self._get_connection()
        try:
            data = [(o.cycle_id, o.symbol, o.agent_name, o.mu, o.sigma, o.confidence, self._serialize_json(o.metadata)) for o in outputs]
            conn.executemany('''
            ...
            logger.error(f"insert_model_outputs failed: {e}")
            return 0
        ...
        try:
            data = [
                (
                    ...
                    self._serialize_json(d.trailing_params) if d.trailing_params else None,
                    self._serialize_json(d.reason_codes), d.data_quality_score,
                    d.provider_confidence, d.mu_hat, d.sigma_hat, d.conviction,
                    self._serialize_json(d.metadata) if d.metadata else None
                )
                for d in decisions
            ]
            conn.executemany('''
                INSERT INTO decisions (
                    ...
                    conviction, metadata_json
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', data)
            conn.commit()
            return len(decisions)
        except Exception as e:
            logger.error(f"insert_decisions failed: {e}")
            return 0
    ...
    def insert_orders(self, orders: List) -> int:
        ...
        try:
            ...
            logger.error(f"insert_orders failed: {e}")
            return 0

    def get_consecutive_skips(self) -> Dict[str, int]:
        """Calculate consecutive skips for all symbols using Python aggregation for SQLite compatibility."""
        conn = self._get_connection()
        # Use ID DESC to ensure true chronological ordering even with identical timestamps
        query = """
        ...
        try:
            ...
            for row in rows:
                ...
                if sym != current_symbol:
                    if current_symbol is not None:
                        res[current_symbol] = consecutive
                    current_symbol = sym
                    consecutive = 0
                    stopped = False
                
                if not stopped:
                    if decision.startswith('SKIP_') or decision == 'REJECT':
                        consecutive += 1
                    else:
                        stopped = True
            ...
            logger.error(f"get_consecutive_skips failed: {e}")
            return {}
