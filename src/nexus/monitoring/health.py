...
class HealthMonitor:
    ...
    def _send_escalation_alerts(self):
        """Send multi-channel escalation alerts (Institutional Standard)."""
        try:
            timestamp = datetime.now().isoformat()
            recent_anomalies_str = self._format_recent_anomalies()
            ...
            # 2. Slack Notification (Production)
            slack_url = os.getenv("SLACK_WEBHOOK_URL")
            if slack_url:
                self._send_slack_alert(slack_url, alert_msg)
            else:
                logger.warning("SLACK_WEBHOOK_URL not set, skipping Slack alert")

            # 3. PagerDuty Integration (Production)
            pd_key = os.getenv("PAGERDUTY_ROUTING_KEY")
            if pd_key:
                self._trigger_pagerduty(pd_key, f"Critical Circuit Breaker: {len(self.anomaly_history)} anomalies")
            else:
                logger.warning("PAGERDUTY_ROUTING_KEY not set, skipping PagerDuty alert")

            # 4. Email Notification (via SMTP)
            smtp_host = os.getenv("SMTP_HOST")
            if smtp_host:
                self._send_email_alert(alert_msg)

            logger.info("Escalation alerts sent successfully across all active channels")
    ...
    def _trigger_pagerduty(self, routing_key: str, summary: str):
        ...
        try:
            ...
            payload = {
                ...
                "payload": {
                    "summary": summary,
                    "source": "MiniQuantFund-HealthMonitor",
                    "severity": "critical"
                }
            }
    ...
    def _send_email_alert(self, message: str):
        ...
        try:
            ...
            if not smtp_user or not recipients: return

            msg = MIMEText(message)
            msg['Subject'] = "CRITICAL: Mini Quant Fund - Circuit Breaker Tripped"
            msg['From'] = smtp_user
