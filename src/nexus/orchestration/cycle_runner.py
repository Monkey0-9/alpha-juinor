...
from mini_quant_fund.features.compute import FeatureComputer, compute_z_temporal_ae
from mini_quant_fund.agents.meta_brain import MetaBrain, SymbolDecision, DECISION_BUY, DECISION_SELL, DECISION_HOLD, DECISION_REJECT

from mini_quant_fund.risk.engine import RiskManager, RiskDecision
from mini_quant_fund.data_intelligence.quality_agent import QualityAgent, QualityResult
from mini_quant_fund.data_intelligence.provider_bandit import ProviderBandit
...
class CycleOrchestrator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Initialize Meta-Brain with aggressive Top 1% settings
        self.meta_brain = MetaBrain(
            gamma=0.5,           # More aggressive Kelly
            max_position_size=0.15, # Larger positions
            allow_short=True,     # Enable shorting for higher returns
            min_confidence_threshold=0.2, # Lower threshold to capture more signals
            **comp_kwargs
        )

        # Initialize agents from registry
        from mini_quant_fund.alpha_families.registry import get_alpha_families
        self.alpha_families = get_alpha_families()
        logger.info(f"[INIT] Loaded {len(self.alpha_families)} alpha families from registry")
    ...
    def _fetch_symbol_prices(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            router = DataRouter()
            df = router.get_price_history(symbol, self.start_date, self.end_date, allow_long_history=True)
            return df
        except Exception as e:
    ...
    def _validate_quality(self, price_data: Dict[str, pd.DataFrame]) -> Dict[str, QualityResult]:
        ...
        for symbol, df in price_data.items():
            ...
            # Link provider info
            result.provider = df.attrs.get('provider', 'unknown') if hasattr(df, 'attrs') else 'unknown'
    ...
    def _run_all_agents(
        ...
        """
        Run all registered alpha families for all symbols.
        """
        agent_outputs = {}
        regime_context = self._get_risk_state() # Use risk state as regime context
        ...
        for symbol in self.universe:
            ...
            # Run each real alpha family
            for alpha_family in self.alpha_families:
                try:
                    # Robust call: first arg positional, others keyword
                    # This ensures compatibility across different agent signatures
                    result = alpha_family.generate_signal(
                        df,                            # data / market_data
                    ...
                    if result and result.get('confidence', 0.0) > 0:
                        # Extract mu/sigma or signal
                        mu = result.get('mu', result.get('signal', 0.0))
                        sigma = result.get('sigma', 0.1)
                        
                        symbol_outputs.append({
                            'agent_name': alpha_family.name,
                            'mu': mu,
                            'sigma': sigma,
                            'confidence': result.get('confidence', 0.0),
                            'metadata': result.get('metadata', result) # Pass result as metadata if no metadata key
                        })
                except Exception as e:
                    logger.error(f"Alpha {alpha_family.name} failed for {symbol}: {e}")
    ...
    def _load_skipping_history(self) -> Dict[str, int]:
        """Load skipping history from mini_quant_fund.database for execution decision validation"""
        try:
            return self.db.get_consecutive_skips()
        except Exception as e:
            logger.error(f"Failed to load skipping history from database: {e}")
            return {}
    ...
    def _generate_orders(
        ...
        # Load skipping history for checks from mini_quant_fund.database
        skipping_history = self._load_skipping_history()
    ...
    def _persist_results(
        ...
        try:
            ...
            # 4. Persist features
            from mini_quant_fund.database.schema import FeatureRecord
            feature_records = []
            ...
            from mini_quant_fund.database.schema import DataQualityRecord
            quality_records = []
            for symbol, quality in quality_results.items():
                record = DataQualityRecord(
                    ...
                    validation_flags=quality.reasons,
