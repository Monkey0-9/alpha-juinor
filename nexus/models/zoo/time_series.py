...
class PyTorchLSTMModel:
    ...
    def __init__(self, sequence_length: int = 10, hidden_dim: int = 32, epochs: int = 15):
    ...
    def _build_sequences(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        df_copy = df.copy()
        df_copy['target'] = np.where(df_copy['close'].shift(-1) > df_copy['close'], 1, 0)
    ...
    def fit(self, df: pd.DataFrame) -> bool:
        ...
        if len(X_seq) < 20:
            logger.warning("Insufficient sequence samples for LSTM; falling back to tree model.")
            return self.tree_fallback.fit(df)
        ...
        try:
            ...
            class LSTMModule(nn.Module):
                def __init__(self, in_d, h_d):
                    super().__init__()
                    self.lstm = nn.LSTM(in_d, h_d, num_layers=2, batch_first=True, dropout=0.1)
            ...
            self.is_trained = True
            logger.info("PyTorchLSTMModel trained successfully on %d sequence samples.", len(X_seq))
            return True
        ...
        try:
            ...
            class TransformerModule(nn.Module):
                def __init__(self, in_d, d_m):
                    ...
                    encoder_layer = nn.TransformerEncoderLayer(d_model=d_m, nhead=2, dim_feedforward=32, batch_first=True)
            ...
            logger.warning("TemporalTransformerModel fit error: %s. Using fallback.", exc)
            return self.tree_fallback.fit(df)
