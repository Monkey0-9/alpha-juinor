...
def check_missing_values(df: pd.DataFrame, threshold: float = 0.05) -> Tuple[bool, pd.DataFrame]:
    """
    ...
    for col in cols_to_check:
        if col in clean_df.columns:
            ...
            if missing_pct > threshold:
                logger.error(f"Column {col} has {missing_pct:.2%} missing values (threshold {threshold:.2%}).")
                passed = False
    ...
    if len(clean_df) < len(df):
        logger.warning(f"Dropped {len(df) - len(clean_df)} rows due to unfillable missing values.")
        passed = False
