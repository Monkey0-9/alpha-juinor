...
class ParquetWriter:
    ...
    def write_ohlcv(self, df: pd.DataFrame, source: str) -> None:
        ...
        if not required_cols.issubset(df.columns):
            raise ValueError(f"DataFrame is missing required columns: {required_cols - set(df.columns)}")
        ...
        for symbol in symbols:
            ...
            for ym in sym_df['year_month'].unique():
                ...
                if file_path.exists():
                    ...
                    ym_df.sort_values('timestamp').to_parquet(file_path, engine='pyarrow', index=False)
