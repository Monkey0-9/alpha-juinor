...
class VolatilityRegimeAdapter:
    ...
    def update(self, returns: np.ndarray) -> str:
        ...
        hist_vol = float(np.std(returns)) if len(returns) > self.window * 3 else recent_vol
...
class PositionManager:
    ...
    def evaluate_exit(self, symbol: str, current_price: float, avg_entry_price: float, pnl_pct: float, history: pd.DataFrame) -> bool:
        ...
        if not history.empty and len(history) > 1:
            ret = float(history["close"].pct_change().dropna().iloc[-1]) if len(history["close"].pct_change().dropna()) > 0 else 0.0
        ...
        if pnl_pct <= Config.STOP_LOSS_THRESHOLD:
            logger.info("Closing %s: Fixed Stop Loss Hit (%.2f%%)", symbol, pnl_pct * 100)
            return True

        if pnl_pct >= Config.TAKE_PROFIT_THRESHOLD:
            logger.info("Closing %s: Take Profit Reached (%.2f%%)", symbol, pnl_pct * 100)
            return True
        ...
        if peak_profit_pct >= Config.TRAILING_PROFIT_LOCK:
            ...
            trailing_lock = peak_profit_pct * 0.5 if vol_regime != "HIGH" else peak_profit_pct * 0.35
            if current_profit_pct < trailing_lock:
                logger.info("Closing %s: Trailing Profit Lock (peak=%.2f%%, now=%.2f%%)", symbol, peak_profit_pct * 100, current_profit_pct * 100)
                return True
        ...
        if atr > 0:
            ...
            if current_price < stop_level:
                logger.info("Closing %s: ATR Trailing Stop (vol_regime=%s, mult=%.1f, curr=%.2f, stop=%.2f)", symbol, vol_regime, stop_mult, current_price, stop_level)
                return True
        ...
        if self._entry_times[symbol] > max_bars:
            logger.info("Closing %s: Maximum holding period reached (%s bars)", symbol, max_bars)
            return True
