...
class VWAPExecutionModel:
    ...
    def schedule(self, total_qty: int, volume_profile: List[float]) -> List[int]:
        profile = np.array(volume_profile[-self.min_bins:]) if len(volume_profile) >= self.min_bins else np.ones(self.min_bins)
...
class ImplementationShortfallModel:
    ...
    def optimal_participation(self, alpha: float, volatility: float, spread: float) -> float:
        if volatility < 1e-9:
            return 0.1
        expected_cost = spread / 2
        alpha_urgency = abs(alpha) * self.urgency
        participation = alpha_urgency / (expected_cost + volatility * self.alpha_decay + 1e-9)
...
class ExecutionAgent:
    ...
    def select_execution_model(self, market_state: np.ndarray, alpha: float = 0.0, volume_profile: Optional[List[float]] = None) -> ExecutionPlan:
        if market_state is None or len(market_state) < 5:
            return ExecutionPlan(action=1, order_type="MARKET", size_pct=0.5, urgency="NORMAL")
        ...
        spread_bps = float(market_state[0]) * 10000 if len(market_state) > 0 else self.default_slippage
        volume_ratio = float(market_state[1]) if len(market_state) > 1 else 1.0
        participation = self.shortfall_model.optimal_participation(alpha, volatility, spread_bps / 10000)

        if volatility > 0.04 or spread_bps > 10:
            return ExecutionPlan(action=0, order_type="WAIT", size_pct=0.0, urgency="LOW")
        if abs(momentum) > 0.03 and alpha * momentum > 0:
            return ExecutionPlan(action=2, order_type="MARKET", size_pct=min(1.0, participation * 2), urgency="HIGH")
        if spread_bps < 3 and volume_ratio > 1.5:
            if volume_profile and len(volume_profile) >= 13:
                return ExecutionPlan(action=1, order_type="VWAP", size_pct=participation, urgency="MEDIUM", twap_duration=13)
            return ExecutionPlan(action=1, order_type="MARKET", size_pct=participation, urgency="MEDIUM")
        if spread_bps < 5:
            return ExecutionPlan(action=1, order_type="TWAP", size_pct=participation * 0.7, urgency="NORMAL", twap_duration=10)
        return ExecutionPlan(action=0, order_type="LIMIT", size_pct=participation * 0.3, urgency="LOW")

    def get_action(self, market_state: np.ndarray, alpha: float = 0.0, volume_profile: Optional[List[float]] = None) -> int:
    ...
    def learn(self, reward: float) -> None:
        ...
        logger.debug("Execution feedback — Reward: %.4f, Cumulative avg: %.4f, Decisions: %d", reward, avg, self.decision_count)