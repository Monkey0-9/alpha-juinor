...
class RegimeModel:
    ...
    def update(self, returns: pd.Series, vix: float = 20.0) -> Dict[str, float]:
        ...
        # Likelihood 0 (Bull)
        # Bull: Low Vol (<0.20) and Positive Ret
        l0_vol = max(0.01, 1.0 - (recent_vol / 0.20)) # 1.0 at 0 vol, 0.0 at 0.20 vol
        l0_ret = max(0.0, recent_ret + 0.02) * 10
        likelihoods[0] = l0_vol + l0_ret

        # Likelihood 1 (Bear)
        # Bear: High Vol (~0.25) and Negative Ret
        l1_vol = max(0.01, 1.0 - abs(recent_vol - 0.25) / 0.15) # Peak at 0.25
        l1_ret = max(0.0, -recent_ret) * 10
        likelihoods[1] = l1_vol + l1_ret

        # Likelihood 2 (Panic)
        # VIX is a strong proxy, plus extreme vol
        l2_vix = 1.0 if vix > 35 else (vix / 35.0)**2
        l2_ret = max(0.0, -recent_ret * 20) # Panic needs huge drop
