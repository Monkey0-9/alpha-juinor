...
class LiveEngine:
    ...
    def __init__(self, provider, handler, risk_manager, initial_capital):
        ...
        self.executor = ThreadPoolExecutor(max_workers=min(32, 10)) # Adjust based on complexity
    ...
    async def run_once_async(self, tickers: List[str], strategy_fn: Callable):
        ...
        logger.info(f"Fetching market data in parallel for {len(tickers)} tickers...")
